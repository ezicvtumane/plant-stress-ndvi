...
* **Автоматическая телеметрия микроклимата**: опрос Sensirion SHT30, вычисление дефицита упругости водяного пара (VPD, кПа), температуры и влажности воздуха в боксе;
* **Двухшаговый измерительный конвейер (Wizard)**:
  1. *Шаг 1*: Стробированный двухволновой импульсный снимок NoIR (660 нм / 850 нм) с вычислением карты NDVI;
  2. *Шаг 2*: Автоматический перехват термограмм тепловизора UNI-T UTi120S через OCR Tesseract, ввод массы кассеты с весов и расчет $\Delta T = T_{\text{leaf}} - T_{\text{air}}$;
* **Мультиспектральная матрица**: параллельный синхронный вывод 4 каналов (660 нм Deep Red, 850 нм NIR, карта NDVI 3×3, ИК-термограмма);
* **Экспорт данных**: мгновенная выгрузка журнала измерений в `.CSV` и генерация полного научно-технического отчета в `.PDF`.
...
```mermaid
flowchart TD
    subgraph Phase1["ЭТАП 1: Сравнительный стресс-скрининг (3 кассеты, 27 растений)"]
        P1_C1["Кассета 1: КОНТРОЛЬ<br/>(Оптимальный полив)"] --> P1_R1["NDVI = 0.74, ΔT = -1.2 °C<br/>Эталонная норма вида"]
        P1_C2["Кассета 2: ЗАСУХА<br/>(Без полива 0..96 ч)"] --> P1_R2["Калибровочная кривая угасания<br/>T_alert = 40 ч (скрытый стресс)<br/>T_wilt = 72 ч (макросимптомы)"]
        P1_C3["Кассета 3: СОЛЬ (NaCl 1.0%)<br/>(Осмотический стресс)"] --> P1_R3["Влажность почвы >80% (мокрая!)<br/>Но ΔT > +1.8 °C, спад NDVI<br/>(Победа над почвенными датчиками)"]
    end

    subgraph Phase2["ЭТАП 2: Валидация гипотезы обратимости и «точки невозврата» (3 кассеты, 27 растений)"]
        P2_C4["Кассета 4: КОНТРОЛЬ (Этап 2)<br/>(Параллельный эталон полива)"] --> P2_R1["NDVI = 0.74, ΔT = -1.1 °C<br/>Стабильная норма микроклимата"]
        P2_C5["Кассета 5: РАННЕЕ СПАСЕНИЕ<br/>(Полив на 40-м ч по сигналу станции)"] --> P2_R4["Коэффициент репарации K_rec = 98.2%<br/>Транспирация включилась за 2 ч<br/>100% сохранение биомассы!"]
        P2_C6["Кассета 6: ПОЗДНЕЕ СПАСЕНИЕ<br/>(Полив на 72-м ч при увядании)"] --> P2_R5["Коэффициент репарации K_rec = 54.1%<br/>Необратимый некроз тканей<br/>Потеря до 45% урожайности!"]
    end

    Phase1 -->|Определение порогов T_alert и T_wilt| Phase2
```
...
$$\kappa_{\text{rec}} = \frac{\text{NDVI}_{\text{после}}}{\text{NDVI}_{\text{исходный}}} \times 100\%$$
...
│       ├── 0_Raskroy_Fanery_760x760.svg <- Карта раскроя листа фанеры 760х760 мм
│       ├── 1_Kub_Kryshka_Top.svg        <- Верхняя панель (сенсоры, диоды, кулер)
│       ├── 3_Kub_Bokovaya_Levaya.svg    <- Стенка с эмблемой «Агробиотехнологии»
│       ├── 6_Kub_Dvertse_Sliding.svg    <- Фасадная дверца с логотипами БВ и Агро
│       └── README_LASER.md              <- Технологические режимы резки фанеры 4 мм
├── src/
