...
* **Автор проекта**: **Ковалева Алиса Ивановна** (Kovaleva Alisa), учащаяся 10 класса
...
* **Пакетный замер триады кассет (3-в-1) с ArUco-сортировкой и ручным дублированием**:
  * Кабель тепловизора подключается **всего один раз** в конце серии (или данные вносятся вручную с экрана UTi120S без проводов);
  ...
  * **Отказоустойчивость**: при повреждении или загрязнении маркера доступен мгновенный выбор когорты из выпадающего списка (ручной fallback);
  * **Синхронный ввод**: масса с весов, температура листа с экрана тепловизора и влажность субстрата вводятся в едином компактном ряду на шаге съемки;
...
flowchart TD
    ...
    subgraph Phase2["💧 ЭТАП 2: Валидация репарации и спасения (3 кассеты, 27 растений)&nbsp;&nbsp;&nbsp;&nbsp;"]
        direction TB
        P2_C4["🌱 <b>Кассета 4</b><br/>ЭТАЛОН<br/>(Оптимум)&nbsp;&nbsp;&nbsp;&nbsp;"] --> P2_R4["NDVI = 0.74<br/>ΔT = -1.1 °C<br/>(Параллельный эталон)&nbsp;&nbsp;&nbsp;&nbsp;"]
        P2_C5["💧 <b>Кассета 5</b><br/>РЕПАРАЦИЯ<br/>(Полив 40 ч)&nbsp;&nbsp;&nbsp;&nbsp;"] --> P2_R5["<b>K_rec = 98.2%</b><br/>Тургор за 2 часа<br/><b>100% спасено</b>&nbsp;&nbsp;&nbsp;&nbsp;"]
        P2_C6["⚠️ <b>Кассета 6</b><br/>КРИТИЧ. СТРЕСС<br/>(Полив 72 ч)&nbsp;&nbsp;&nbsp;&nbsp;"] --> P2_R6["<b>K_rec = 54.1%</b><br/>Некроз тканей<br/><b>Потеря ~45%</b>&nbsp;&nbsp;&nbsp;&nbsp;"]
    end
...
| **Этап 2** | **К4: Эталон (Оптимум)** | Параллельный эталон полива | $\text{NDVI} = 0.74$, $\Delta T = -1.1\ ^\circ\text{C}$ | Независимый параллельный контроль микроклимата |
| **Этап 2** | **К5: Репарация (~40ч)** | Полив на 40-м ч (сигнал комплекса) | $\kappa_{\text{rec}} = \mathbf{98.2\% \pm 1.8\%}$ | Тургор за 2 ч, **100% урожая сохранено** |
| **Этап 2** | **К6: Критический стресс (~72ч)** | Полив на 72-м ч (при увядании) | $\kappa_{\text{rec}} = \mathbf{54.1\% \pm 5.3\%}$ | Необратимый некроз, **потеря ~45% биомассы** |
