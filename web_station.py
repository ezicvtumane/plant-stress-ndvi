...
def auto_mount_uti():
    ...
    os.system('sudo mkdir -p /media/uti120s')
    for dev in ['/dev/sdc1', '/dev/sdb1', '/dev/sda1', '/dev/sdd1', '/dev/sde1', '/dev/sdc', '/dev/sdb', '/dev/sda']:
        if os.path.exists(dev):
            os.system(f'sudo mount {dev} /media/uti120s 2>/dev/null')
            if os.path.exists(UTI_DIR):
                try:
                    if len(os.listdir(UTI_DIR)) > 0:
                        break
...
def index(msg: str = ''):
    ...
    if uti_files:
        ...
        file_options = ''.join(file_options_list)
    else:
        file_options = '<option value=\"\">[Тепловизор не подключен к USB]</option>'

    t_now = int(time.time())
    status_banner = ''
    if msg == 'done':
        status_banner = '<div style=\"background:#10b981;padding:10px;border-radius:8px;font-weight:bold;margin-bottom:12px;text-align:center;\">✅ Оптический замер сохранен! Сделайте снимок тепловизором.</div>'
    elif msg == 'linked':
        status_banner = '<div style=\"background:#0284c7;padding:10px;border-radius:8px;font-weight:bold;margin-bottom:12px;text-align:center;\">⚡ Выбранная термограмма успешно привязана к замеру! Температура распознана.</div>'
...
<head>
    ...
    <style>
        body {{ font-family: system-ui, sans-serif; background: #0b1120; color: #f8fafc; margin: 0; padding: 15px; }}
        ...
        .grid {{ display: grid; grid-template-columns: 390px 1fr; gap: 15px; }}
        ...
        .preview-img {{ width: 100%; height: 180px; border-radius: 4px; border: 1px solid #475569; background: #000; object-fit: contain; }}
...
<div class=\"container\">
    ...
    <div class=\"grid\">
        ...
        <div class=\"card\">
            ...
            <h2 style=\"margin-top: 15px;\">Журнал замеров серии</h2>
